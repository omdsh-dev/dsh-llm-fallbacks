#!/usr/bin/env bash
#
# verify-dsh-patch.sh — 校验目标 dsh 源码树中 role 组 patch 的效果（出现 / 消失）。
#
# 检查 role 组两个 patch（dsh-agent + dsh-tool-subagent）探针（文件存在即检查，
# 缺失记为 SKIP；不会把缺失误判为通过或失败）：
...
if [[ "$FAIL" -eq 1 ]]; then
  echo "== 校验失败：patch 标记（role 组，期望${EXPECT_LABEL}）未满足" >&2
  exit 1
fi

echo "== 校验通过：patch 标记（role 组，期望${EXPECT_LABEL}）满足"
