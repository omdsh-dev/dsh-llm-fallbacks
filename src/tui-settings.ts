  ...
  return {
    ...
    fields: [
      ...
      },
      {
        ...
        hint: 'JSON array of model selectors; the last entry must be an official model (deepseek-official/deepseek-flash or deepseek-official/deepseek-pro). Pro is a legal tail but not yet available in the catalog — the settings card shows it disabled.',
        hintDescriptions: { zh: '模型选择器 JSON 数组；末项必须是官方模型（deepseek-official/deepseek-flash 或 deepseek-official/deepseek-pro）。Pro 是合法链尾但目录暂未提供——设置卡中显示为禁用。', en: 'JSON array of model selectors; the last entry must be an official model (deepseek-official/deepseek-flash or deepseek-official/deepseek-pro). Pro is a legal tail but not yet available in the catalog — the settings card shows it disabled.' },
        group: 'chain',
        kind: 'text',
