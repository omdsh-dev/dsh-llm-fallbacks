...
export function validateConfigPatch(patch: unknown): void {
  ...
  for (const key of Object.keys(patch)) {
    ...
    // model — V4 Flash / V4 Pro / V41 Flash XOR (the card's 默认模型
    // panel); leading entries (默认降级链) are the ordered walk before
    // that last-resort fallback. Empty / non-official tail rejected on save.
    if (key === 'rootChain') {
      const chain = (patch as Record<string, unknown>)[key]
      if (chain !== null && chain !== undefined && (!Array.isArray(chain) || !isAllDayConforming(chain))) {
        throw new Error(
          'dsh-llm-fallbacks: rootChain must end with exactly one official model (deepseek-official/deepseek-v4-flash, deepseek-official/deepseek-v4-pro, or deepseek-official/deepseek-flash)',
        )
      }
    }
  }
