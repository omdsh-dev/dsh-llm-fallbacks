...
> **发布状态**：包**尚未发布到 npm registry**——首个版本（`0.1.0-alpha.2`）待发布（发布流水线已就绪，发布动作等待 Trusted Publishing 授权）。本节的 registry 安装命令在首个 release PR merge 前均为**预告**；在此之前请用 [git 安装](#git-安装当前可用)。
