...
| `rootChain` | string[] | `[]` | **主代理 — the all-day chain**. Must END with exactly one official model (`deepseek-official/deepseek-flash` **or** `deepseek-official/deepseek-pro` — the card's 默认模型 panel); leading entries (the card's 默认降级链 block) are walked first, then the tail is the last-resort fallback. An empty chain or a chain whose last entry is not an official model (including the retired `deepseek-v4-flash` / `deepseek-v4-pro` ids) warns at startup, keeps the v0.2.2 fallback-only walk, and refuses the virtual picker delegate, but cannot be saved as-is through the card/gateway. Used whenever no `timeSlots` row matches |
...
The all-day `rootChain` is a chain: its LAST entry (the tail — the card's 默认模型 panel) must be **exactly one** official model — `deepseek-official/deepseek-flash` XOR `deepseek-official/deepseek-pro`; leading entries (the card's 默认降级链 block) are optional, ordered walk targets before that last-resort fallback. The card renders the tail as an exclusive 2-choose-1 panel (required, not removable) and the rest as a selector list above it (UI order = walk order), and both the card and the gateway **reject** any value whose last entry is not an official model on save — an empty chain or a legacy non-official-tail `rootChain` cannot be saved as-is (no migration wizard; pick one of the two models). At load, a non-conforming tail earns ONE startup warn, slot rows stay inert, the virtual picker row refuses the delegate, and the v0.2.2 fallback-only walk continues over the raw chain.

**Retired V4 ids**: the previous `deepseek-official/deepseek-v4-flash` / `deepseek-official/deepseek-v4-pro` tails are no longer legal. A saved `rootChain` whose tail is a V4 id is now non-conforming: startup warns, time-slot rows and the virtual picker row go inert, and the card/gateway refuse to save it until a legal tail (Flash or Pro) is picked.

**`deepseek-pro` availability**: Pro is a legal *selector* but its model is not yet served by the 0.1.5-rc.2 catalog — requests to it fail at the provider until the gateway enables the id (the same advisory-catalog risk dsh documents for its own defaults). The settings card shows the Pro option disabled with a "not yet available" note; a hand-set Pro tail still reads back as selected. The plugin does not probe catalog availability: a chain containing `deepseek-pro` dispatches to it like any other exact entry, and on the virtual route the `stream()` delegate serves the first dispatchable exact head of the effective chain, so a chain with a working entry before Pro still routes to that earlier entry.
