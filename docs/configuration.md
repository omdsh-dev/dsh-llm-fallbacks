...
- **Readback**: `/fallbacks config` prints the composed `fallbacks` namespace — first line `Fallbacks config: enabled` / `disabled`, then trigger codes, root chain, roles (`N — id (chain: n)` per role, long lists truncated with `…`), cooldown, revert policy, max switches/step, always-mode cap, presets, and the file-only edit hints. Bare `/fallbacks` stays the session diagnostic (origin / role / chain / recent switches / cooldown) — the two surfaces are distinct from their first line.
