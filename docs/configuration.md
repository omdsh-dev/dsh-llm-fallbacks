...
- **Trim upgrade note (honest copy)**: versions before the trim also declared `designer` and `librarian`. After upgrading, rows saved by those earlier versions keep their persona and survive untouched, but they are no longer in the declared batch — they now show source `user` (the `User` badge, no seeded-row treatment), **even though the operator never wrote those rows**. They are ordinary config rows again (the label means "no live declaration", not "operator-written"); delete them by hand if unwanted.
