...
- **Picker label**: the catalog row's `id` stays `Auto`; its `name` (what the host ModelSelect trigger and menu render) is live — `Auto: <slot_label>[<model>]`, e.g. `Auto: Liang Peak[deepseek-v4-flash]` or `Auto: all-day[deepseek-v4-flash]`. Slot label comes from `resolveSlotState` (same source as the 分时切换 log). Bare `Auto` when the all-day chain is non-conforming (no dispatchable head). The host reloads the catalog on picker open, so the label refreshes then.
