...
| `rootChain` | string[] | `[]` | **主代理 — the all-day chain**. Must END with exactly one official model (`deepseek-official/deepseek-flash` **or** `deepseek-official/deepseek-v4-pro` — the card's 默认模型 panel); leading entries (the card's 默认降级链 block) are walked first, then the tail is the last-resort fallback. An empty chain or a chain whose last entry is not an official model (including the retired `deepseek-v4-flash` and the never-served legacy `deepseek-official/deepseek-pro` ids) warns at startup, keeps the v0.2.2 fallback-only walk, and refuses the virtual picker delegate, but cannot be saved as-is through the card/gateway. Used whenever no `timeSlots` row matches |
...
**Seeded rows**: a companion plugin may also auto-provision role rows through the service seeding API — a seeded role is a plain `roles.list` row (`{ id, persona }`, two keys only): seeds never write `chain` / `fallback` (a new seeded role keeps an empty chain until you fill it), its persona can be reverted to the currently declared seed default via the service or the TUI `/fallbacks config revert-seed <role-id>` command (the web settings card offers no revert), and the card shows a source badge (`bundled` / the declaring set's name, `external` when unnamed / `User`). The card presents a seeded row as reference material, not editable config: no id field (the collapse title carries the immutable id, R2) and the persona renders as a read-only single-line brief with an expandable full view (no persona editor, no revert button), while chain and fallback stay editable. See [docs/consumer-api.md](consumer-api.md) → Role seeds. The plugin itself also auto-provisions 5 bundled preset roles on apply by default (see [Preset roles](#preset-roles-presets-key)) — same seed semantics, same source badge.
...
- **Not gated by `enabled`**: the self-declaration fires on apply regardless of the feature switch — even a default install (`enabled: false`) materializes the 5 preset rows. The only zero-declaration path is `presets: 'none'`.
- **Upgrade behavior**: with the default configuration, the first `apply` after upgrading materializes the 5 rows into `roles.list`; each row shows a **source badge** in the settings card (existing capability — no extra configuration). Setting `presets: 'none'` stops further declarations but does **not** retract already-materialized rows — delete them by hand if you want them gone. **Honest limitation**: a hand-deleted row is re-materialized by the next `apply` (the plugin cannot distinguish "operator deleted" from "never existed").
- **Trim upgrade note (honest copy)**: versions before the trim also declared `designer` and `librarian`. After upgrading, rows saved by those earlier versions keep their persona and survive bundled declares untouched, but they are no longer in the bundled producer's slice — the plugin no longer has a live declaration for them. They now show source `user` (the `User` badge, no seeded-row treatment) only when no live producer still declares the id, **even though the operator never wrote those rows**. Only then are they ordinary editable config rows again (the label means "no live declaration", not "operator-written"); a companion's live declaration keeps them seeded and read-only with the winning producer's source label (`external` when unnamed, otherwise its registered set name), including when the mstar merge-preserve pattern copies currently seeded non-own ids verbatim. Delete them by hand if unwanted.
- **Per-row source**: every row carries a provenance label, read-time derived and never persisted — preset rows read back with source `bundled`; a row no live declaration covers reads `user`. The contract → [docs/consumer-api.md](consumer-api.md) → Per-row source.
- **Same-name operator rows are never overwritten**: a row the operator already defined keeps its persona — the declaration is flagged with a loud `logger.warn` (`'persona-source'` conflict, seed semantics unchanged) and the row still derives `seeded=true`, so the source badge is available. The preset persona is only ever applied to a brand-new row.
...
- **Virtual-route carve-out (explicit `FallbacksChain` / `Auto` spawn)**: a subagent explicitly spawned on the plugin's virtual provider is user-authorized for that route (spec D2 — explicit selection); its requests route through the virtual adapter's thin delegate to the configured effective chain head (see [FallbacksChain in the model picker](#fallbackschain-in-the-model-picker)). That internal delegation is the route's documented purpose and is outside D1's selection semantics — D1 governs provider/model selection the plugin makes, not delegation inside an explicitly chosen virtual route — so the delegated head is not intersected with the allowlist. Every plugin-originated selection above (authorized-head detection, inject, failure switch) stays allowlist-constrained.
...
`reasoningEffort` on **every** `overrideConfig` path (role-inject, pending-switch apply, always-cap switch) follows the upstream 0.1.2 `resolveChildAgentOptions` routeChanged rule:
...
- **Picker label**: the catalog row's `id` stays `Auto`; its `name` (what the host ModelSelect trigger and menu render) is live — `Auto: <displayName>[<slot>]`, e.g. `Auto: DeepSeek Flash[Liang Peak]`. The display name comes from the head provider's catalog (`listModels`), not the model id, so the same id on different platforms stays distinguishable. Slot label comes from `resolveSlotState`. Bare `Auto` when the all-day chain is non-conforming. The host reloads the catalog on picker open, so the label refreshes then.

- **Select-is-primary is a delegation, not a rewrite**: selecting `FallbacksChain` / `Auto` means "use the configured chain as the root primary". A root-origin seed of the virtual pair is served **unchanged** (plan `model-change-notice-loop` Task 1 removed the former `agent/request` rewrite); the virtual adapter's `stream()` delegate is what dispatches the **first dispatchable exact `provider/model`** head of the effective chain at request time — `effectiveHeadOf` → `resolveEffectiveChain(config, now, tz)` from `src/time-slots.ts`, slot-aware when a `timeSlots` row matches, `rootChain` otherwise; the resolver is the single source and there is deliberately no `rootChain[0]` fallback branch. Serving the selection unchanged is what keeps the durable `request/header` equal to the session selection, so the host `model-selection` notice fires on a genuine selection change instead of being re-armed on every step. Every consumer that has to compare a **recorded** route against the chain anchors the virtual pair at the served head instead (`anchorServedRoute` in `src/index.ts`; `servedRouteFor` for rule matching) — left on the virtual pair, the failure walk would treat the head that just failed as a fresh candidate and key cooldown / step bookkeeping on the picker row, the rule-6 half-open **success observer** could never close the head's circuit (its durable `assistant/message` source records the request route), and a dispatch-time **role rule** keyed on the real head would stop matching a delegated child whose recorded pair is the virtual row.
- **Misconfiguration surfaces at the request, not at `agent/request`**: the two routing-path warns that shipped with the removed rewrite ("the all-day rootChain is not conforming … no primary override" and "the effective chain has no exact head … no primary override") are **gone** with it. A non-conforming all-day chain is therefore observed through the failed request: the delegate throws `UNDISPATCHABLE_EFFECTIVE_HEAD` (non-conforming all-day, wildcard selector, malformed selector, or a self-route to `FallbacksChain/*`) or `EMPTY_EFFECTIVE_CHAIN` (empty chain), and the runtime normalizes that into a terminal error finish. There is no longer a plugin log line at the moment the row refuses to serve.
...
- **Thin delegate on every origin**: the row is served by the same `stream()` delegate for a root agent and for a subagent session that inherits the selection — with the rewrite gone there is no origin gate on the route itself (the removed rewrite was root-only, so subagent-origin virtual seeds were already served by the delegate). Subagent role resolution and injection keep their semantics: a delegated child records the parent's `request/header` pair — the virtual row when the parent runs on the virtual route — while the delegate serves it the chain head, and dispatch-time rule matching accepts **either** pair (the optional served route on `resolveRole`), so a rule keyed on the real head keeps matching and a rule that names the virtual row keeps matching too. The virtual row stays a thin delegate, never a second routing engine. The walk anchor (`anchorServedRoute`) has **no** origin gate either: a subagent-origin virtual route is anchored at the served head too, while a subagent's own walk tail stays the raw `rootChain`. That asymmetry is an **accepted gap** — gating the anchor by origin would make the decision's `from` name a route the delegate never served. A second accepted gap sits in the clock: the anchor stamps its **own** `new Date()` while `decide` re-resolves the effective chain with a fresh one, so a time-slot boundary landing between those two reads can name a different head for the same step. The window is bounded by one decision for the `decide` / `anchorServedRoute` pair, and by roughly **one request duration** for the rule-6 **success observer**, which stamps its own `new Date()` when the `assistant/message` arrives rather than reusing the head resolved at request time — a slot boundary crossing inside a long request can therefore anchor that observation to a different head than the one served. Both windows self-heal on the next request and neither can be closed from inside the plugin. A third accepted gap sits at the **outer** ownership pass of a virtual-route session: a durable assistant message whose recorded source names a **real** provider loses its `replayState` before the virtual adapter's `stream()` is entered, because that pass targets the virtual adapter — which does not own the historical provider — so the runtime's ownership check strips the envelope first. It is reachable whenever a session carries real-route envelope history, which is the very scenario this route creates (e.g. switching to `FallbacksChain` / `Auto` inside a session that already has pi-ai-backed history), and it is a **regression introduced by the change that removed the root rewrite**, not a pre-existing condition: the removed rewrite made the root request carry the head pair, so that pass aimed at the head adapter and kept history recorded on that head — only the cross-adapter-instance rotation subset predates it. Evidence strength, stated exactly: the post-change half is **executed** (the real-route boundary arm in `tests/virtual-adapter.spec.ts` asserts the envelope is withheld); the "the removed rewrite kept it" half is **reasoned** from the deleted rewrite plus the runtime's instance-ownership rule, not measured. It cannot be closed from inside the plugin — the envelope is already absent from the delegated history — so the seams are upstream: the loop records the served route rather than the request route, or the ownership pass consults the envelope's own provenance (the same shape this route's own re-stamp uses).
...
- **Tail-conformance gate for the delegate**: a successful `stream()` delegate requires the all-day chain to be **tail-conforming** — its LAST entry must be exactly one official model (`isAllDayConforming` in `src/time-slots.ts`; leading 默认降级链 entries are walked first). A legacy multi-model or empty chain keeps the row visible, but the delegate refuses it: `UNDISPATCHABLE_EFFECTIVE_HEAD` for a non-conforming multi-model chain, `EMPTY_EFFECTIVE_CHAIN` for an empty one. The routing-path warn that used to accompany the refusal left with the rewrite (see the misconfiguration bullet above).
...
- **Metadata and retry policy follow the head**: `resolveModel('Auto')` proxies the current effective head's model info (modalities / context window / reasoning) when resolvable, with a permissive identity default otherwise (never throws). `providerRetryPolicy('FallbacksChain')` returns the **effective head provider's** policy resolved through the live runtime (`src/virtual-adapter.ts`), so a user's `llm-deepseek.retryPolicy` — including `mode: 'always'` — behaves on the virtual route exactly as it does on the head the delegate dispatches; an unresolvable head, a vanished runtime, or an unregistered head provider degrades to `undefined` (the host's own default) and never throws. Two consequences, stated one-directionally: the host captures that policy **once**, inside `prepareRoutes` at `registerAdapter` time (`@deepseek-ai/dsh-llm` `lib/index.js`), so a later `llm-deepseek.retryPolicy` edit — or a slot-driven head-**provider** rotation — is only reflected after the plugin re-registers; and because the capture is a snapshot, a rotated head can leave the captured policy **more** permissive than the live head's. Before this change the virtual route always answered with the permissive default, so the snapshot is strictly better than that pre-fix state only in the direction measured here — it is not a blanket improvement over the live policy. Retry **accounting** follows the route the runtime sees, which on the virtual route is the virtual pair: `llm/retry` events are keyed by `FallbacksChain`, and the plugin's own cap counting reads the same key.
...
The all-day `rootChain` is a chain: its LAST entry (the tail — the card's 默认模型 panel) must be **exactly one** official model — `deepseek-official/deepseek-flash` XOR `deepseek-official/deepseek-v4-pro`; leading entries (the card's 默认降级链 block) are optional, ordered walk targets before that last-resort fallback. The card renders the tail as an exclusive 2-choose-1 panel (required, not removable) and the rest as a selector list above it (UI order = walk order), and both the card and the gateway **reject** any value whose last entry is not an official model on save — an empty chain or a legacy non-official-tail `rootChain` cannot be saved as-is (no migration wizard; pick one of the two models). At load, a non-conforming tail earns ONE startup warn, slot rows stay inert, the virtual picker row refuses the delegate, and the v0.2.2 fallback-only walk continues over the raw chain.

**Retired id**: the previous `deepseek-official/deepseek-v4-flash` tail is no longer legal — the catalog dropped the model (its sibling `deepseek-v4-pro` is the legal Pro tail again, see below). A saved `rootChain` whose tail is `deepseek-official/deepseek-v4-flash` is now non-conforming: startup warns, time-slot rows and the virtual picker row go inert, and the card/gateway refuse to save it until a legal tail (Flash or Pro) is picked.

**Pro availability**: the 0.1.7-rc.2 official default catalog serves the Pro tail as `deepseek-official/deepseek-v4-pro` (re-verified against rc.2; rc.1 re-introduced the id) (alongside the default `deepseek-official/deepseek-flash`), and the settings card offers it as a selectable option — no disabled state, no availability caveat. A hand-written legacy `deepseek-official/deepseek-pro` tail was never catalog-served nor selectable: it reads back as a non-conforming tail (one startup warn, inert slot rows, unselected in the card) — no migration wizard; re-pick the tail as `deepseek-official/deepseek-v4-pro`. The plugin does not probe catalog availability: a chain containing `deepseek-official/deepseek-v4-pro` dispatches to it like any other exact entry, and on the virtual route the `stream()` delegate serves the first dispatchable exact head of the effective chain, so a chain with a working entry before Pro still routes to that earlier entry.
...
fallbacks:
  ...
  triggerCodes:
    ...
    - QUOTA
    - RATE_LIMIT
  rootChain:                     # All-day chain: LAST entry is 默认模型 (official model)
    - anthropic/claude-3-5-sonnet          # leading entries = 默认降级链 (walked first)
    - deepseek-official/deepseek-flash  # last = Flash / Pro last-resort fallback
...
- `timeSlots` rows rotate the effective root chain by wall-clock windows: first matching row wins (its chain replaces the all-day chain), no match → the all-day `rootChain`; the all-day row is always last and required (exactly one official model — the example above shows one of the two).
...
- **Action command**: `/fallbacks config revert-seed <role-id>` restores a seeded role's persona to its declared seed default and prints the outcome — the web settings card presents seeded personas read-only (no revert affordance), so the command is this action's only surface.
