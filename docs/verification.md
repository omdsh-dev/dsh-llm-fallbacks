...
driven by the real host type surface (registry peer `@deepseek-ai/*@0.1.0-rc.8`, no in-repo type shims). The no-op regression
...
- **Real-type contract**: the type layer does not use hand-written `peer-stubs/` — `autoInstallPeers` resolves the real
  `@deepseek-ai/*@0.1.0-rc.8` peers from the npm registry (user-level `~/.npmrc` auth, no local link farm);
...
1. **Preflight check**: record `dsh --version` (snapshot); the plugin-side peer dependencies resolve from the npm registry
   (`@deepseek-ai/*@0.1.0-rc.8`, no source tree needed).
