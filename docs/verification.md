...
- The harness double (`tests/support/model-selection-stub.ts`) installs the real `agent/pre-step` notice listener and carries a **copy of the
  upstream notice string** — a DSH rewording breaks that assertion **without** any plugin defect, so the literal has to be re-synced deliberately.
...
Result: **52 files / 1141 tests passed, 1 skipped** (`corepack pnpm@11.21.0 test`, vitest run); `pnpm build` (`tsc -p tsconfig.build.json` emits JS first (standard decorator downgrade `__esDecorate`) → tsdown host bundle →
