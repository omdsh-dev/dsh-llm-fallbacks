...
driven by the real host type surface (registry peer `@deepseek-ai/*@0.1.2-alpha.1`, no in-repo type shims). The no-op regression
...
1. **Preflight check**: record `dsh --version` (snapshot); the plugin-side peer dependencies resolve from the npm registry
   (`@deepseek-ai/*@0.1.2-alpha.1`, no source tree needed).
