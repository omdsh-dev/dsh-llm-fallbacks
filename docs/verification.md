...
### 1. Test matrix (unit + integration + client + host gateway + command + release/consumer tooling; 52 files / 1149 tests passed, 1 skipped)
...
| time slots + virtual picker (T5 ext) | `time-slots.spec.ts` / `time-slots-runtime.spec.ts` / `virtual-adapter.spec.ts` / `tui-settings.spec.ts` / `tui-client.spec.ts` / `context-window.spec.ts` / `override.spec.ts` | 43 / 12 / 30 / 51 / 15 / 9 / 14 | all-day tail conformance (Flash / Pro XOR; the retired V4 ids rejected), slot resolver first-match windows, virtual FallbacksChain adapter delegate (served route, no rewrite) + effective-head retry-policy proxy, TUI settings schema readback, context-window cap skip, request override path |
...
- The accepted gaps that live in production code (the anchor's origin gate and its own clock) are recorded under
  [docs/configuration.md](docs/configuration.md) § FallbacksChain in the model picker — they are documented, not tested.

Result: **52 files / 1149 tests passed, 1 skipped** (`corepack pnpm@11.21.0 test`, vitest run); `pnpm build` (`tsc -p tsconfig.build.json` emits JS first (standard decorator downgrade `__esDecorate`) → tsdown host bundle →
