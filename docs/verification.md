...
| client（T5） | `fallbacks-store.spec.ts` | 61 | 设置页描述符读/写（redactSecrets 面、expectedRevision 冲突保护、settings-conflict 状态）、chain/rule 行编辑往返、状态块最近切换提取（sessions.history 事件面）、controller 生命周期 |
...
- **真实 dsh 源码树只读校验**：四个 patch 对 `$DSH_SOURCE_DIR`（真实 `$DSH_HOME/source/current`）
  `git apply --check` 均通过（只读，树保持 clean、零写入）；`--reverse --check` 在 pristine
  树上按预期失败。
- **沙箱拷贝全流程**（工作区内重建 `.dsh-patch-test/`，pre-image blob 与真实树一致；
  T6 扩展至四 patch + 暴露组探针后复验）：
  `apply --check → apply（agent → tool-subagent → settings → apiproxy）→ verify（role + 暴露组
  src 探针全 PASS）→ 再次 apply（幂等跳过）→ revert（逆序 apiproxy → settings → tool-subagent →
...
1. 应用 dsh 本体 patch（真实安装写操作，需用户执行；四个 patch，幂等）：

   ```sh
   cd <插件仓库目录>
   scripts/apply-dsh-patch.sh --check   # 只读检查
   scripts/apply-dsh-patch.sh           # git apply 四 patch → 增量构建
...
4. 回滚验证：`scripts/revert-dsh-patch.sh --check` → `scripts/revert-dsh-patch.sh`（逆序
   回滚四 patch）→ `scripts/verify-dsh-patch.sh --absent`（role + 暴露组标记消失）。
...
2. **应用四 patch（幂等）**：
   ...
   scripts/apply-dsh-patch.sh           # 应用 agent → tool-subagent → settings → apiproxy → 增量构建
   scripts/verify-dsh-patch.sh          # 8 探针（4 src + 4 构建产物）→「校验通过」
   ```

   预期：apply 输出四 patch `[skip] 已应用`（已应用）或 `== 应用 ...`（新应用），退出 0；
   verify 输出 `== 校验通过：patch 标记（role + 暴露组，期望出现）满足`。
3. **重建 dsh 受影响包**：`apply-dsh-patch.sh` 内置完成（`tsc -b` 四包 + `tsdown host`）；
...
2. **预期①（暴露生效）**：不再显示「未注册」——页面渲染骨架（nav / 介绍 / 只读状态块 /
   `enabled` 开关 / 保存 / 恢复默认），命名空间描述符可读写。
...
5. **保存** → UI saving → ready（携带 `expectedRevision`，无冲突横幅）。
...
7. **立即生效证据（AC-1 核心）**：**不重启 host、不重载页面**——先确认 host PID/启动
   ...
   → **保存后下一次故障即切换 = 无需重启会话**。
8. **读回证据**：重载页面 → 显示服务端真值（`enabled` 保持 ON、链行在、revision 正常）；
...
3. **冲突保护抽查（可选）**：另一会话/直接编辑 `settings.yaml` 后保存 → 冲突横幅 +
   「重新加载」按钮，不静默覆盖。
