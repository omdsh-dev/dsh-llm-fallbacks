...
- A working dsh runtime environment (`$DSH_HOME`, defaults to `~/.dsh`); development-time type checking and tests resolve the `@deepseek-ai/*@0.1.2-alpha.1` peer dependencies from the npm registry (registry auth token — see "Authentication (pnpm 11)" below).
...
- **pnpm version**: pnpm 11.21+ (this project's stack). Since 11.21 the minimum-release-age supply-chain gate is enabled by default; the `minimumReleaseAgeExclude` table in `pnpm-workspace.yaml` is a **pnpm-maintained** exemption table — the whole 0.1.2-alpha.1 line (and cordis 4.0.1) was published on the same day and is auto-listed; **do not delete the block manually**: a resolved lockfile hard-fails without the exemptions (`ERR_PNPM_MINIMUM_RELEASE_AGE_VIOLATION`; re-resolve with `pnpm clean --lockfile`).
