...
- A working dsh runtime environment (`$DSH_HOME`, defaults to `~/.dsh`); development-time type checking and tests need the `@deepseek-ai/*@0.1.7-rc.2` peer dependencies resolved (see "Development-time `@deepseek-ai/*` peer resolution" below).
...
`@deepseek-ai/*` is a private package family: at runtime it is provided by the host dsh box as a bundle (`peerDependencies` contract; `@deepseek-ai/*` is externalized at tsdown build time). During development the real packages (`0.1.7-rc.2`) resolve in one of two supported ways:

- **npm registry (current)**: 0.1.7-rc.2 is on the npm registry — `autoInstallPeers: true` in `pnpm-workspace.yaml` + an auth token in the user-level `~/.npmrc` — `pnpm install` pulls in the peer dependencies automatically, so type checking, tests, and navigation all run against the real published code (no local link farm).
...
- **Version**: `peerDependencies` is pinned to `^0.1.7-rc.2` (aligned with the dlx host's 0.1.7-rc.2); bump the peer version in sync after a dsh upgrade.
- **pnpm version**: pnpm 11.21+ (this project's stack). Since 11.21 the minimum-release-age supply-chain gate is enabled by default; the `minimumReleaseAgeExclude` table in `pnpm-workspace.yaml` is a **pnpm-maintained** exemption table covering the published dsh package line (rc versions and cordis 4.0.2), with the `0.1.7-rc.1` / `0.1.7-rc.2` entries covering that line's first registry publishes; **do not delete the block manually**: a resolved lockfile hard-fails without the exemptions (`ERR_PNPM_MINIMUM_RELEASE_AGE_VIOLATION`; re-resolve with `pnpm clean --lockfile`).
- **Client store seam**: the client half's one runtime VALUE import is `@deepseek-ai/dsh-client-store` (the snapshot-store engine — a frozen loader-table platform module, so the client bundle stays external on it). The published 0.1.7-rc.2 package ships `lib/index.js` + `lib/types/*.d.ts`, so the PR worktree's vitest graph runs in **registry mode**: the store VALUE import resolves through the package's exports map and no alias is needed. The local-link alternative (a sibling dsh checkout linked into `node_modules`, tsc-built into `lib/types/` only) is still supported — in that mode `vitest.config.ts`'s probe detects the missing `lib/types/index.js` and aliases the VALUE-imported packages to the linked tree.
- **Upstream manifest gap (0.1.7-rc.2)**: the published `@deepseek-ai/dsh-client-ui-primitives@0.1.7-rc.2` bare-imports `@deepseek-ai/dsh-util-code-language` at runtime (its markdown code highlighting) without declaring it in the published manifest — the source tree declares it only as a devDependency, and the host box resolves it from its own full package set, so only npm-registry dev installs hit the gap. The `packageExtensions` block in `pnpm-workspace.yaml` patches the manifest so the vitest/dev graph resolves the import; remove the block once an upstream release declares the dependency.
...
Restart the dsh web session for the uninstall to take effect. After uninstalling, the plugin no longer participates in any request/error path. Note: the plugin writes no durable `fallbacks/switch` session events (issue #52 — the apply()-time registration was proven ineffective), so new sessions are never affected. Sessions written by older plugin versions that contain such events **cannot** be repaired by an `ignorable` flag — the released session-format migration chain refuses unknown event types even when marked ignorable — so the plugin repo's `pnpm repair:session-logs` tool reports them and writes nothing by default — only the explicit lossy `--drop-legacy-events` opt-in removes such rows (see the README's [Repair existing sessions](../README.md#repair-existing-sessions) section).
