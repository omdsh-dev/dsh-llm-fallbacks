...
describe('registry peer contract (0.1.2-alpha.5 from npm, no link farm)', () => {
  ...
  it('every @deepseek-ai/dsh-* peer is pinned to ^0.1.2-alpha.5', () => {
    for (const [name, range] of Object.entries(root.peerDependencies ?? {})) {
      if (name.startsWith('@deepseek-ai/dsh-')) {
        expect(range, name).toBe('^0.1.2-alpha.5')
      }
    }
  })
