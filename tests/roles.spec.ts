...
const ROLE_IDS = new Set(['code-review', 'root-openai', 'anthropic-only', 'gpt4o-only'])
...
describe('resolveRole', () => {
  ...
  it('treats a missing origin as root (root agents carry no origin)', () => {
    ...
    expect(resolveRole(agent, rules, new Set(['root-chain']))).toBe('root-chain')
  })
  ...
  it('matches an origin+provider+model combo rule when all patterns fit', () => {
    ...
    expect(resolveRole(agent, rules, new Set(['subagent-openai-gpt4o']))).toBe('subagent-openai-gpt4o')
  })
  ...
  it('skips a combo rule when any one pattern differs', () => {
    ...
    expect(resolveRole(agent, rules, new Set(['subagent-openai-gpt4o']))).toBe('inherit')
  })
  ...
  it('respects rule order: first match wins', () => {
    ...
    expect(resolveRole(agent, rules, new Set(['first', 'second']))).toBe('first')
  })
