...
| [architecture-patterns/dsh-tui-client-seams.md](architecture-patterns/dsh-tui-client-seams.md) | fallbacks-tui-client | dsh-TUI 前端插件挂载点地图（tuiCommandTrees provider 形状/生命周期、registry merge 自动浮现、settings 命名空间 + 文件写面、bundle composition、命令注册 hint 校验坑、活体验证法含探针插件 + 专用 dsh CLI 安装） | Active |
